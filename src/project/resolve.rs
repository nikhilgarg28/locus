...
pub fn resolve(program: Program, source: &SourceFile) -> (Program, Graph, Vec<Diagnostic>) {
    // Project loading checks each original file before adding synthetic package
    // modules. The flat elaboration API needs the same hygiene check.
    let reserved: Vec<_> = crate::lexer::lex(source)
        .tokens
        .into_iter()
        .filter(|t| {
            t.kind == crate::lexer::TokenKind::Name
                && source
                    .slice(t.span)
    ...
    if !reserved.is_empty() {
        return (Program::default(), Graph::default(), reserved);
    }
    resolve_units(
        vec![Unit {
            program,
