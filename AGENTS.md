...
- Current documentation fences declare a language and mode: `locus check`, `locus run`, `locus reject Lxxxx`, or a non-executable form such as `text prose kernel-grammar`. Run examples need `//~ run:` expectations; rejected examples need diagnostic codes; prose exemptions need an honest reason. Do not exempt an executable example just because it fails.
