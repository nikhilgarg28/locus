...
Unsigned machine integers have Nat as their canonical model, signed integers have Int, and bool has Bool. Every physical type has at most one model. Ordinary logical named-field access observes the receiver first, then selects a model field. `model!(place)` explicitly selects a physical read path before observing its final value; it works even when the enclosing struct has no model. Structural derivation is opt-in. `as` between logical values performs a defined logical conversion, including exact Nat-to-Int widening, rather than choosing a second model. Proof types still name propositions or use `@(e)` as shorthand for `@prop!(e)`; arrays retain square brackets.
...
Machine types remain runtime types. Int and Nat are Logical and erase. Unsigned machine observations produce Nat, signed observations produce Int. `model!(n)` makes the boundary explicit; `n as Int` in logical code widens an unsigned observation. Logical helpers operate on these values, not an executable integer passed under another name.
...
- Runtime integer casts preserve Rust's meaning. A logical cast first observes its operand through the canonical model, then performs a supported logical conversion. Nat widens to Int; converting Int to Nat needs checked nonnegativity evidence. Logical-to-runtime extraction is forbidden.
- Machine addition, subtraction, multiplication and signed negation return an exact representable result or panic in every build. Pre-operation safety evidence permits plain Rust operators; otherwise emission uses checked methods followed by expect. no_panic requires evidence. Successful return establishes exactness even without a static safety proof; post-operation facts cannot justify omitting that same check. Explicit wrapping methods remain modular.
...
- A primitive operation is an explicit checking-IR statement. Check its safety evidence in the incoming context before adding its result. With evidence, emit a plain Rust operator; without it, emit checked arithmetic that panics on overflow. Both interpreters use that same rule under either Rust build setting. Exact normal-return facts are available in either case. A no_panic promise requires evidence rather than a runtime fallback.
...
A physical type T declares `impl Model for T { type Logic = M; logic fn model(&self) -> Self::Logic { ... } }`. There is one canonical implementation for T, and M must be Logical. A named shared parameter is equivalent to &self. `model!(place)` observes an explicit physical path; ordinary logical named-field access first observes its receiver. `#[derive(Model)]` creates a fieldwise NameModel only when every physical field has a model. Missing models are diagnosed rather than invented. Slice observers cover compatible array/vector borrows, so overlapping implementations are rejected. Generic model traits remain separate future work.
...
A model field is erased because its type is Logical, regardless of whether its enclosing record is runtime. Neither logic let nor a Ghost wrapper is needed. For explicit historical observations, bind the selected model value with ordinary let. old!(x) continues to select entry state in signatures; model! applies the canonical model to the selected entry path. There is no generic source operation that snapshots arbitrary runtime T into a secretly logical occurrence of the same T.
...
The kernel retains native Int and arithmetic certificates. Source Nat is currently a checked Logical product of Int and nonnegativity evidence. Primitive model relations and the translation of executable operations into model facts remain on the trusted path, with differential tests supporting the correspondence. A library's declaration of Model alone supplies no unchecked facts.
...
- prop!(e) is the built-in spelling of logic { holds(e) }; e must be a logical Bool. old!(x) selects the entry observation under Mutation and references. Explicit model! observations and ordinary let provide named snapshots; the earlier Ghost<T>-producing snapshot! is superseded.
...
- recurse!(evidence, call), rewrite!(eq, h), unfold!(f, h), and fold!(f, h) take the same spelling. recurse! wraps what must be, syntactically, a recursive call, and the type expected of its evidence is computed from that call. unfold! and fold! select a logical function or an explicit model!(place) observation. The result type of rewrite! is computed by substitution. None of the four names is reserved any longer.
