...
`src/project/specs.rs` completes `spec mod` and `spec type` against local definitions before module lowering. Headers are never kernel declarations. It compares source signature tokens including proofs, inherits effect promises and exposes only declared members. The resolver additionally rejects extra type implementations after canonical path resolution, closing alias and cross-module bypasses. Definition bodies then use the existing checked pipeline. This surface-contract matching is part of compiler correctness; it adds no kernel axiom. Native imports remain a separate planned trust boundary.
