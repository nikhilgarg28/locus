...
`project/traits.rs` resolves concrete trait/type pairs, checks ownership and completeness, substitutes associated items and compares binder-normalized contracts. It instantiates inherited defaults with the selected implementation and lowers members to uniquely named ordinary methods. Scope metadata guides static method lookup; explicit qualified selections retain both type and trait identity. All resulting bodies pass the existing ownership, proof, logical-cycle and IR checks. No abstract trait contract is installed as a kernel axiom.

The native metadata adapter normalizes a supported Rust trait interface, retains provenance and validates it against actual Cargo metadata using a compiler probe. Rust aliases share one trait identity. Imported defaults require local bodies; unsupported traits remain inspectable. Export emits compatible physical trait signatures and concrete forwarding implementations. Logical members, proof slots and promises cannot cross that interface. Defaults checked for particular Locus implementations are not emitted as reusable Rust defaults for unchecked implementors. Trait selection, signature matching and Rust forwarding are part of the whole-compiler correctness boundary; the kernel is unchanged.

## Borrowed dynamic dispatch

`elab/dynamic.rs` resolves each used physical interface and its fixed associated types. `exec/dynamic.rs` validates dispatch tables against checked bodies. Opaque snapshot declarations in the kernel admit no constructor or projection and introduce no proof axiom. `typed/dynamic.rs` builds shared-reference conversion and dispatch adapters, which reuse ordinary call evaluation and provenance checking. The erased checker independently validates their physical signatures.

Both interpreters carry the hidden concrete value and table identity. The Rust backend emits real `&dyn` interfaces, specialized by fixed associated-type bindings, with forwarding implementations of the already checked bodies. These internal Rust traits are not a public ABI for the original source traits; object import/export remains deferred. Logical observers and proof-bearing dynamic interfaces require a separate design.
