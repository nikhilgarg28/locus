...
4. Erased fields are removed from generated Rust and positions are renumbered (section 11). The alternative is a zero-sized placeholder per erased field, which keeps positions stable and remains uniform once generic parameters can be instantiated with proof types.
