...
pub(super) fn step(
    lock: Lock,
    bounded: @within_limit(model!(lock.failures) as Int),
    event: Event,
) -> (next: Lock, @within_limit(model!(next.failures) as Int)) {
    match event {
        Event::Right => {
            let next = Lock { failures: 0, open: true };
            (next, within_limit::Bounds @ prove!((model!(next.failures) as Int) <= 3))             // computed: 0 <= 3
        }
        Event::Wrong => {
            if lock.failures < 3 {
                let fits = prove!(model!(lock.failures) as Int + 1 <= u32::MAX as Int); // arithmetic, from lock.failures < 3
                let next = Lock { failures: lock.failures + 1, open: false };   // the obligation of + is fits: exact
                (next, within_limit::Bounds @ prove!((model!(next.failures) as Int) <= 3))         // arithmetic
            } else {
...
#[no_panic] #[no_io]
pub fn run(attempts: u32, correct: u32) -> (last: Lock, @within_limit(model!(last.failures) as Int)) {
    let mut lock = Lock { failures: 0, open: false };
    let mut ok: @within_limit(model!(lock.failures) as Int) = within_limit::Bounds @ prove!((model!(lock.failures) as Int) <= 3); // computed
