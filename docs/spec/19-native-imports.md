...
Every used supported function is checked against Cargo's actual native metadata with a Rust function-pointer signature assertion. A rustdoc-only item, including one exposed by `cfg(doc)`, cannot authorize a call. The check establishes a physical signature, never behavioral correctness or a model.
