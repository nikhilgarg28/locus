...
[Traits](20-traits.md) support associated types and generic bounds. General bound integration for the compiler-owned Model interface remains deferred under [LOC-266](../roadmap/generics.md#LOC-266). Model registration remains a dedicated compiler-owned operation. Model destinations are named logical types, including concrete generic instances. Structural derivation currently supports structs; enums need manual models. Anonymous tuples have no implicit fieldwise model: use `model!(pair.0)` to select a physical component. These restrictions avoid silently selecting an abstraction or adding runtime work.
