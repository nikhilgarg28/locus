...
## Inspect and call Rust

A plain native import is separate from `use` of another package's Locus source. It adds no proof assumptions. Start by inspecting the selected Cargo interface:

~~~sh prose shell-commands
locus import dependency_alias --manifest-path Cargo.toml --offline
locus import dependency_alias --manifest-path Cargo.toml --features extra --json
locus import dependency_alias::module --manifest-path Cargo.toml --out target/native.json
~~~

In source, `import dependency_alias::function as local_name;` binds the native function; `import crate::helper;` binds the owning package's Rust helper. The [native import manual](spec/19-native-imports.md) defines supported calls and diagnostics. Complete Cargo fixtures in [native-import tests](../tests/native_imports.rs) compile generated callers and preserve observable effects.

The importer uses the installed stable compiler and matching rustdoc, without installing nightly. Experimental JSON output is enabled only for that rustdoc subprocess. The adapter pins its upstream format and rejects unknown versions. `--features`, `--all-features`, `--no-default-features`, `--target`, `--locked` and `--offline` select the build whose interface is inspected. Use those same settings when compiling the output. Imported metadata includes all item kinds, even when their use is not implemented.

The current extraction performs a fresh isolated `cargo check` of the host. The host must already compile: invoking imports from a host build script that itself needs this generated code is a diagnosed cycle. Rustc wrapper environment overrides and ambiguous host/target library instances also fail explicitly. Stage-aware build-script imports and sysroot metadata are tracked follow-ups; ordinary Locus-only build scripts continue to work. Neither inspection output nor build receipts are theorem certificates.
