...
Target-sensitive checks exercise both 32-bit and 64-bit pointer layouts, including collection indices, arithmetic bounds and proof-store separation. Host-generated Rust is checked under both overflow modes. Simulation at another width is not execution on that architecture; generated Rust additionally rejects a mismatched pointer width. Target discovery and propagation are included in the whole-compiler trusted inventory. Tuple/unit structs reuse nominal product proofs, with separate tests for erasure, constructor shape, private fields and moves.
...
Static trait bounds add a source-selection obligation: a generic call must keep the declared trait identity when specialized. The bound checker validates concrete implementations and associated types before ordinary body/kernel checking; it does not introduce proof axioms. Regression tests cover shadowing by inherent methods, conditional implementations, failed proof contracts, erased type identities, native aliases and generated Rust. Generic bodies are still checked per instantiation; universal generic proof checking remains [LOC-265](roadmap/generics.md#LOC-265).

One independent-interpreter coverage gap is pinned under [LOC-268](roadmap/generics.md#LOC-268): indirect logical callable values are accepted and erase correctly, but the checking interpreter does not yet evaluate all such calls. Tests of that form do not count as successful differential comparisons.

Borrowed dynamic dispatch adds independently checked interface/table shapes and opaque snapshots without logical constructors. Its tests compare runtime-selected implementations in both interpreters and warning-denied generated Rust, and reject forged tables, misplaced unsized values, logical contracts, invalid borrows and escaping references. Dynamic calls make no observer or effect promises. This first tier does not establish correctness of proof-bearing objects or arbitrary Rust implementations supplied through a public object ABI.
